...
class WorklistsScreen(Screen):
    ...
    def _show_week(self, worklist_id: int):
        ...
        if view.locked:
            ...
            ttk.Button(bar, text="Amend", style="Accent.TButton",
            ...
            m.add_command(label="Edit name / notes",
                          command=lambda: self._edit_worklist(worklist_id, view))
            m.add_command(label="Regenerate recurring",
            ...
            m.add_separator()
            m.add_command(label="Archive worklist",
                          command=lambda: self._archive(worklist_id))
            menu["menu"] = m
    ...
    def _task_menu(self, event, task_id):
        ...
        m.add_command(label="Edit task",
                      command=lambda: self._edit_task(task_id))
        m.add_command(label="Manage assignees",
                      command=lambda: self._manage_assignees(task_id))
        m.add_separator()
    ...
    def _amend(self, worklist_id):
        ...
        if not vals:
            return
        new_id = run_write(
            self, commands.amend_worklist(
                worklist_id, amendment_reason=vals["amendment_reason"],
    ...
    def _archive(self, worklist_id):
        vals = forms.prompt(self, "Archive worklist",
                            [forms.text("reason", "Reason")],
                            submit_label="Archive")
        if vals is None:
            return
        run_write(
            self, commands.archive_worklist(worklist_id, vals.get("reason") or ""),
            confirm=("Archive worklist",
                     "Hide this worklist from the active lists? Its data is "
                     "kept."),
    ...
    def _manage_assignees(self, task_id):
        ...
        if data["locked"]:
            messagebox.showinfo("Locked",
                                "Amend the worklist before editing tasks.",
                                parent=self)
            return
        ...
        for a in data["assignments"]:
            ...
            if not a["is_poic"]:
                ttk.Button(row, text="Make lead",
                           command=lambda aid=a["id"]: self._do_assignment(
                               dlg, task_id,
                               commands.set_assignment_poic(task_id, aid))
                           ).pack(side="right", padx=2)

        ttk.Button(dlg, text="+ Add assignee", style="Accent.TButton",
                   command=lambda: self._add_assignee(dlg, task_id)).pack(
            anchor="w", pady=(4, 0))
    ...
    def _add_assignee(self, parent_dlg, task_id):
        people = read(commands.active_people_choices())
        vals = forms.prompt(parent_dlg, "Add assignee", [
        ...
        if not vals.get("person_id") and not vals.get("external_poic_name"):
            return
        run_write(self, commands.add_assignment(
            task_id, person_id=vals.get("person_id"),
    ...
    def _remove_task(self, task_id):
        vals = forms.prompt(self, "Remove task",
                            [forms.text("reason", "Reason")],
                            submit_label="Remove")
        if vals is None:
            return
        ...
        run_write(
            self, commands.archive_task(task_id, vals.get("reason") or ""),
            pii_texts=(vals.get("reason"),),
            on_done=lambda: self.app.show("worklists", worklist_id=wl_id)
            if wl_id else self.app.show("worklists"))
    ...
    def _carry_over(self, worklist_id):
        ...
        if data is None or data["locked"] or not data["candidates"]:
            messagebox.showinfo("Carry-over",
                                "No pending tasks to carry over.", parent=self)
            return
        dlg = tk.Toplevel(self)
        dlg.title("Carry-over review")
        dlg.configure(bg=theme.BG, padx=16, pady=14)
        dlg.transient(self)
        ttk.Label(dlg, text="Pending tasks from earlier weeks",
                  style="H2.TLabel").pack(anchor="w", pady=(0, 8))
        scroller = VScroll(dlg)
        scroller.pack(fill="both", expand=True)
        choices = [("carry", "Carry to this week"),
        ...
        for c in data["candidates"]:
            ...
            card.pack(fill="x", pady=3)
            ttk.Label(card.body, text=c["name"], style="Card.TLabel",
                      font=theme.Fonts().bold).pack(anchor="w")
            meta = []
            if c["assignees"]:
            ...
            var = tk.StringVar(value={"carry": "carry", "discard": "discard",
                                      "manual": "leave"}.get(c["suggested"],
                                                             "carry"))
            actions[c["instance_id"]] = var
            rowf = ttk.Frame(card.body, style="Card.TFrame")
            rowf.pack(anchor="w", pady=(4, 0))
            for val, label in choices:
                ttk.Radiobutton(rowf, text=label, value=val, variable=var
                                ).pack(side="left", padx=(0, 8))

        def apply_all():
            decisions = {iid: {"action": var.get()}
                         for iid, var in actions.items()}
        ...
        ttk.Button(btns, text="Apply", style="Accent.TButton",
                   command=apply_all).pack(side="right", padx=6)
        dlg.grab_set()
