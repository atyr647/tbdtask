...
class TemplatesScreen(Screen):
    ...
    def refresh(self, **_):
        ...
        if not rows:
            empty_state(scroller.body, "No recurring templates yet.").pack(
                anchor="w")
            return
        for t in rows:
            ...
            top.pack(fill="x")
            ttk.Label(top, text=t["name"], style="Card.TLabel",
                      font=theme.Fonts().bold).pack(side="left")
            badge(top, t["recurrence"], status="recurring").pack(
                side="left", padx=8)
            ttk.Button(top, text="Archive",
                       command=lambda tid=t["id"]: self._archive(tid)).pack(
                side="right")
            ttk.Button(top, text="Edit",
                       command=lambda tid=t["id"]: self._edit(tid)).pack(
                side="right", padx=4)
            meta = []
            if t["category"]:
    ...
    def _fields(self):
        ...
        return [
            ...
            forms.choice("carry_over_policy", "Carry-over",
                         [(v, lbl) for v, lbl in commands.CARRY_OVER_POLICIES]),
            forms.choice("rec_kind", "Recurrence",
                         [(v, lbl) for v, lbl in commands.RECURRENCE_KINDS]),
            forms.text("rec_weekdays", "  Weekdays (0=Mon..6=Sun, comma-sep)",
                       help="for 'weekday(s)'"),
            forms.integer("rec_n", "  N (weeks, or Nth)", help="for N-weeks/Nth"),
            forms.integer("rec_weekday", "  Weekday (0=Mon)",
                          help="for N-weeks/Nth"),
    ...
    def _recurrence_from(self, vals):
        ...
        return commands._build_recurrence(
            kind, weekdays=weekdays, n=vals.get("rec_n"),
            weekday=vals.get("rec_weekday"), day=vals.get("rec_day"),
            anchor=vals.get("rec_anchor"))

    def _add(self):
        vals = forms.prompt(self, "New recurring template", self._fields(),
                            initial={"carry_over_policy": "auto_same_person",
                                     "rec_kind": "none"}, submit_label="Create")
        if not vals:
            return
        run_write(self, commands.create_template(
            name=vals["name"], category_id=vals.get("category_id"),
    ...
    def _edit(self, template_id):
        ...
        vals = forms.prompt(self, "Edit template", self._fields(),
                            initial=initial, submit_label="Save")
        if not vals:
            return
        run_write(self, commands.update_template(
            template_id, name=vals["name"], category_id=vals.get("category_id"),
            description=vals.get("description"),
            estimated_hours=vals.get("estimated_hours"),
            carry_over_policy=vals.get("carry_over_policy") or "auto_same_person",
            recurrence=self._recurrence_from(vals),
    ...
    def _archive(self, template_id):
        vals = forms.prompt(self, "Archive template",
                            [forms.text("reason", "Reason")],
                            submit_label="Archive")
        if vals is None:
            return
        run_write(
            self, commands.archive_template(template_id, vals.get("reason") or ""),
