...
class AbsencesScreen(Screen):
    ...
    def _build_list(self, parent):
        ...
        table.set_rows([
            {"_id": r.id, "name": r.person_name, "code": r.code,
    ...
    def _build_calendar(self, parent, cal_start):
        ...
        ttk.Button(nav, text="‹ Earlier",
                   command=lambda: self.app.show(
                       "absences", tab="calendar", cal_start=data.prev_start)
                   ).pack(side="left", padx=2)
        ttk.Button(nav, text="Today",
                   command=lambda: self.app.show("absences", tab="calendar")
                   ).pack(side="left", padx=2)
        ttk.Button(nav, text="Later ›",
                   command=lambda: self.app.show(
                       "absences", tab="calendar", cal_start=data.next_start)
                   ).pack(side="left", padx=2)
        ttk.Label(nav, text=f"{data.start_date.isoformat()} + {len(data.days)} days",
        ...
        for label, span in data.month_groups:
            tk.Label(grid, text=label, bg=theme.PANEL3, fg=theme.TEXT_DIM,
                     font=theme.Fonts().bold, pady=2).grid(
                row=0, column=col, columnspan=span, sticky="nsew", padx=1, pady=1)
            col += span

        # Day-number header
        tk.Label(grid, text="Name", bg=theme.PANEL3, fg=theme.TEXT_DIM, anchor="w",
                 font=theme.Fonts().bold, padx=8, width=NAME_W).grid(
            row=1, column=0, sticky="nsew", padx=1, pady=1)
        for c, d in enumerate(data.days, start=1):
            wd = d.strftime("%a")[0]
            weekend = d.weekday() >= 5
            tk.Label(grid, text=f"{wd}\n{d.day}", bg=(theme.BG2 if weekend else theme.PANEL3),
                     fg=theme.TEXT_DIM, font=small, width=3).grid(
                row=1, column=c, sticky="nsew", padx=1, pady=1)

        # Person rows
        for r, crow in enumerate(data.rows, start=2):
            bgc = theme.PANEL2 if r % 2 else theme.PANEL
            tk.Label(grid, text=crow.name, bg=bgc, fg=theme.TEXT, anchor="w",
                     font=small, padx=8, width=NAME_W).grid(
        ...
        tk.Label(grid, text="% present", bg=theme.PANEL3, fg=theme.TEXT_DIM, anchor="w",
                 font=theme.Fonts().bold, padx=8, width=NAME_W).grid(
            row=foot, column=0, sticky="nsew", padx=1, pady=1)
        for c, pct in enumerate(data.daily_percent_present, start=1):
            ...
            tk.Label(grid, text=f"{pct:g}", bg=theme.PANEL3, fg=col, font=small,
                     width=3).grid(row=foot, column=c, sticky="nsew", padx=1, pady=1)
