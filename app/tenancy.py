...
class TenantScopedMixin:
    ...
    ``org_id`` is nullable in Phase 0 so existing single-tenant rows can be
    backfilled to a "Default Organization" without breaking the running app.
    Phase 3 makes it NOT NULL and turns on Postgres row-level security.
    """

    @declared_attr
    def org_id(cls) -> Mapped[Optional[int]]:
