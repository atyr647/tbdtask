...
from tests.conftest import (
    make_absence_codes, make_person, make_task, make_task_categories,
    make_worklist, set_prd, make_qual,
)
...
def test_recompute_resolves_stale_prd_alert_when_data_changes(session):
    ...
    cur = session.scalars(
        select(M.PersonPrd).where(M.PersonPrd.person_id == p.id,
                                  M.PersonPrd.valid_to.is_(None))
    ).first()
    cur.valid_to = _today()
...
def test_carry_over_pending_alert_fires_on_open_worklist(session):
    ...
    make_task(session, worklist_id=older.id, name="Open from prior week",
              status="in_progress", scheduled_date=_today() - timedelta(days=14))
    target = make_worklist(session, _today())  # current week
    session.commit()

    recompute(session); session.commit()
    rows = [r for r in active_alerts(session)
