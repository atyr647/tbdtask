...
def test_locked_worklist_rejects_task_update(client, session_factory):
    ...
    with session_factory() as s:
        wl = M.Worklist(
            week_starting=monday, name="Locked week", locked=False, org_id=1
        )
        s.add(wl)
        s.flush()
        inst = M.TaskInstance(
            worklist_id=wl.id, name="Original", status="open", org_id=1
        )
        s.add(inst)
        s.commit()
        wl.locked = True
...
def test_locked_worklist_rejects_assignment_changes(client, session_factory):
    ...
    with session_factory() as s:
        ...
        s.add(
            M.PersonRosterStatus(
                person_id=p.id, status="active", valid_from=date.today(), org_id=1
            )
        )
        ...
        s.flush()
        a = M.TaskAssignment(
            instance_id=inst.id, person_id=p.id, is_poic=True, org_id=1
        )
        s.add(a)
        s.commit()
        wl.locked = True
    ...
    resp = client.post(
        f"/tasks/{inst_id}/assignments/{a_id}/delete", follow_redirects=False
    )
    assert resp.status_code == 409
...
def test_amend_clones_into_new_worklist_with_parent_link(client, session_factory):
    ...
    resp = client.post(
        f"/worklists/{wl_id}/amend",
        data={"amendment_reason": "spelling fix", "operator_name": "Op"},
        follow_redirects=False,
    )
    ...
    with session_factory() as s:
        ...
        cloned_assignments = list(
            s.scalars(
                select(M.TaskAssignment).where(
                    M.TaskAssignment.instance_id == cloned.id
                )
            ).all()
        )
